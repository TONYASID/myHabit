...
public final class HabitsStore {
    ...
    /// Добавляет или удаляет текущую дату в trackDates для переданной привычки.
    /// - Parameter habit: Привычка, в которую добавится или удалится новая дата.
    ...
    public func reversetrackDateString(forIndex index: Int) -> String? {
        let datesReverce = Array(dates.reversed())
        guard index < dates.count else {
            return nil
        }
        return dateFormatter.string(from: datesReverce[index])
    }
